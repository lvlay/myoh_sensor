...
#define HDF_LOG_TAG    uhdf_mylight_service
...
static uint32_t ShowHelloworldInfo(struct HdfSBuf *reply)
{
    ...
    HDF_LOGI("%{public}s: mylightdump: print hello world !", __func__);

    return HDF_SUCCESS;

}

static int32_t DumpMylightChannel(struct HdfSBuf *reply)
{
    int32_t ret;
    HDF_LOGI("%{public}s: get mylight dump channel begin", __func__);
...
static int32_t MylightDriverDump(struct HdfSBuf *data, struct HdfSBuf *reply)
{
    uint32_t i;
    uint32_t argv = 0;
    HDF_LOGI("%{public}s: get mylight dump begin xx", __func__);
...
int32_t GetMylightDump(struct HdfSBuf *data, struct HdfSBuf *reply)
{
    HDF_LOGI("%{public}s: get mylight dump begin", __func__);
    int32_t ret = MylightDriverDump(data, reply);
    if (ret != HDF_SUCCESS) {
        HDF_LOGE("%{public}s: get mylight dump failed", __func__);
        return HDF_FAILURE;
    }
