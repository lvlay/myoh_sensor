...
#define HDF_LOG_TAG    uhdf_maniu_service
...
static uint32_t ShowHelloworldInfo(struct HdfSBuf *reply)
{
    ...
    HDF_LOGI("%{public}s: maniudump: print hello world !", __func__);

    return HDF_SUCCESS;

}

static int32_t DumpManiuChannel(struct HdfSBuf *reply)
{
    int32_t ret;
    HDF_LOGI("%{public}s: get maniu dump channel begin", __func__);
...
static int32_t ManiuDriverDump(struct HdfSBuf *data, struct HdfSBuf *reply)
{
    uint32_t i;
    uint32_t argv = 0;
    HDF_LOGI("%{public}s: get maniu dump begin xx", __func__);
...
int32_t GetManiuDump(struct HdfSBuf *data, struct HdfSBuf *reply)
{
    HDF_LOGI("%{public}s: get maniu dump begin", __func__);
    int32_t ret = ManiuDriverDump(data, reply);
    if (ret != HDF_SUCCESS) {
        HDF_LOGE("%{public}s: get maniu dump failed", __func__);
        return HDF_FAILURE;
    }
